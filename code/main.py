    ...
    for snr in snr_arr:
        for p in p_arr:
            ...
            for n_induce in induce_arr:
                ...
                dump_pymc3_model('../model/fitc_N_{0}_p_{1}_scale_{2}_induce_{3}'.format(N, p, snr, n_induce), skim_run_induce, skim_time_induce)
                # Subsample
                skim_model_sub, skim_run_sub, skim_gp_sub, skim_time_sub = SKIM_exact(X[:n_induce, :], y[:n_induce], m0)
                dump_pymc3_model('../model/subsamp_N_{0}_p_{1}_scale_{2}_induce_{3}'.format(N, p, snr, n_induce), skim_run_sub, skim_time_sub)
