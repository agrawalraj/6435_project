...
def do_all_exact(N, p, snr, m0=5):
    X = np.load('../data/synthetic/X_N_{0}_p_{1}_scale_{2}'.format(N, p, snr))
    y = np.load('../data/synthetic/y_N_{0}_p_{1}_scale_{2}'.format(N, p, snr))
    skim_model, skim_run, skim_gp, skim_time = SKIM_exact(X, y, m0)
    dump_pymc3_model('../model/exact_N_{0}_p_{1}_scale_{2}.pkl'.format(N, p, snr), skim_run, skim_time)
...
def do_all_induce(N, p, snr, n_induce, m0=5):
    ...
    dump_pymc3_model('../model/fitc_N_{0}_p_{1}_scale_{2}_induce_{3}.pkl'.format(N, p, snr, n_induce), skim_run_induce, skim_time_induce)
    # Subsample
    skim_model_sub, skim_run_sub, skim_gp_sub, skim_time_sub = SKIM_exact(X[:n_induce, :], y[:n_induce], m0)
    dump_pymc3_model('../model/subsamp_N_{0}_p_{1}_scale_{2}_induce_{3}.pkl'.format(N, p, snr, n_induce), skim_run_sub, skim_time_sub)
